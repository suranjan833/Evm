...
class _NewsViewScreenState extends State<NewsViewScreen> {
  ...
  refreshPosts() async {
    ...
    // if(db.getCheckFilterClicked){
    //   toast("Filter Api Hit");
    // }
    //toast(newsList.length.toString());
    ...
    if (_controller.offset >= _controller.position.maxScrollExtent &&
      ...
      // newsList = newsList+db.getLatestNews;
      // setState(()=>{});
      //toast(newsList.length.toString());
      //toast(newsList.length.toString());
    }
  ...
  Widget build(BuildContext context) {
    bool loader = true;
    return Consumer<AppDataHandler>(builder: (context, data, index) {
      final theme = Theme.of(context).textTheme;
      final ads = data.getAdvertisements;
      final newsList = data.getLatestNews;
      newsList.removeWhere((element) =>
          data.getFlagsList.any((flag) => flag.toString() == element.newsId));
        ...
        child: SizedBox(
          ...
          children: [
            ...
            refreshing
                ...
                : Text(""),
            Provider.of<AppDataHandler>(context, listen: false).subnewcount > 1
                ? Positioned(
